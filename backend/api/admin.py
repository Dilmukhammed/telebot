...
# ── TEMPORARY: Database reset endpoint (DELETE AFTER USE) ──────────────
@router.post("/reset-database")
async def reset_database(
    secret: str = Query(..., description="Must be 'confirm-reset-2026'"),
    db: AsyncSession = Depends(get_db),
):
    """
    ⚠️ DESTRUCTIVE: Clears ALL data and seeds fresh admin + teacher.
    Call with: POST /api/admin/reset-database?secret=confirm-reset-2026
    ...
    tables = [
        "attendance", "audit_logs", "notification_recipients", "notifications",
        "lesson_statuses", "lesson_enrollments", "lessons", "teacher_availability",
        "results", "registrations", "tests", "subjects", "admins", "users",
    ]
    ...
    # Seed admin (separate Admin table for JWT auth)
    import bcrypt
    ...
    teacher = User(
        telegram_id=-2,
        username="tdima01",
        first_name="Dilmukhammed",
        last_name="Turdimuratov",
        role="teacher",
        is_active=True,
        onboarded=True,
    )
    ...
    return {
        ...
        "deleted": deleted,
        "seeded": {
            "admin": "gi_rocke (telegram_id=-1)",
            "teacher": "@tdima01 Dilmukhammed Turdimuratov (telegram_id=-2)",
        },
    }
# ── END TEMPORARY ─────────────────────────────────────────────────────
